...
1. Extract the modules from the image with `jimage` into a `jar` subdirectory:
  `jdk-22\bin\jimage extract --dir=jextract-22\jar jextract-22\lib\modules`
2. Create the jar from the module with `jar`:
  `jdk-22\bin\jar cfM jextract-22\jar\jextract.jar -C jextract-22\jar\org.openjdk.jextract/ .`
    * `c` creates a jar
